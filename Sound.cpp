...
constexpr uint32_t const MIX_SAMPLES =
    1024; // number of samples to mix per call of mix_audio callback; n.b. SDL
...
Sound::Sample::Sample(std::string const &filename) {
  if (filename.size() >= 4 && filename.substr(filename.size() - 4) == ".wav") {
    ...
    throw std::runtime_error(
        "Sample '" + filename +
        "' doesn't end in either \".png\" or \".opus\" -- unsure how to load.");
  }
}
...
std::shared_ptr<Sound::PlayingSample>
Sound::play(Sample const &sample, float play_volume, float pan) {
  std::shared_ptr<Sound::PlayingSample> playing_sample =
      std::make_shared<Sound::PlayingSample>(sample, play_volume, pan, false);
  lock();
...
std::shared_ptr<Sound::PlayingSample> Sound::play_3D(Sample const &sample,
                                                     float play_volume,
                                                     glm::vec3 const &position,
                                                     float half_volume_radius) {
  std::shared_ptr<Sound::PlayingSample> playing_sample =
      std::make_shared<Sound::PlayingSample>(sample, play_volume, position,
                                             half_volume_radius, false);
  lock();
  playing_samples.emplace_back(playing_sample);
  unlock();
  return playing_sample;
...
std::shared_ptr<Sound::PlayingSample>
Sound::loop(Sample const &sample, float play_volume, float pan) {
  std::shared_ptr<Sound::PlayingSample> playing_sample =
      std::make_shared<Sound::PlayingSample>(sample, play_volume, pan, true);
  lock();
  playing_samples.emplace_back(playing_sample);
  unlock();
  return playing_sample;
...
std::shared_ptr<Sound::PlayingSample> Sound::loop_3D(Sample const &sample,
                                                     float play_volume,
                                                     glm::vec3 const &position,
                                                     float half_volume_radius) {
  std::shared_ptr<Sound::PlayingSample> playing_sample =
      std::make_shared<Sound::PlayingSample>(sample, play_volume, position,
                                             half_volume_radius, true);
  lock();
  playing_samples.emplace_back(playing_sample);
  unlock();
  return playing_sample;
...
void Sound::PlayingSample::set_pan(float new_pan, float ramp) {
  if (!(pan.value == pan.value))
    return; // ignore if not in '2D' mode
  Sound::lock();
  pan.set(new_pan, ramp);
  Sound::unlock();
}

void Sound::PlayingSample::set_position(glm::vec3 const &new_position,
                                        float ramp) {
  if (pan.value == pan.value)
    return; // ignore if not in '3D' mode
  Sound::lock();
  position.set(new_position, ramp);
  Sound::unlock();
}

void Sound::PlayingSample::set_half_volume_radius(float new_radius,
                                                  float ramp) {
  if (pan.value == pan.value)
    return; // ignore if not in '3D' mode
  Sound::lock();
...
void Sound::Listener::set_position_right(glm::vec3 const &new_position,
                                         glm::vec3 const &new_right,
                                         float ramp) {
  Sound::lock();
  position.set(new_position, ramp);
...
void compute_pan_from_listener_and_position(glm::vec3 const &listener_position,
  ...
  // note that for a LR fade to sound uniform, sound power (squared magnitude)
...
void step_direction_ramp(Sound::Ramp<glm::vec3> &ramp) {
  if (ramp.ramp < RAMP_STEP) {
    ...
    float angle =
        std::acos(glm::clamp(glm::dot(ramp.value, ramp.target), -1.0f, 1.0f));
...
void mix_audio(void *, Uint8 *buffer_, int len) {
  ...
  assert(len ==
         MIX_SAMPLES *
             sizeof(LR)); // should always have the expected number of samples
  ...
  for (auto si = playing_samples.begin(); si != playing_samples.end();
       /* later */) {
    Sound::PlayingSample &playing_sample =
        **si; // much more convenient than writing ** everywhere.
    ...
    if (!(playing_sample.pan.value == playing_sample.pan.value)) {
      ...
      compute_pan_from_listener_and_position(
          start_position, start_right, playing_sample.position.value,
          playing_sample.half_volume_radius.value, &start_pan.l, &start_pan.r);
    ...
    if (!(playing_sample.pan.value == playing_sample.pan.value)) {
      // 3D panning
      compute_pan_from_listener_and_position(
          end_position, end_right, playing_sample.position.value,
          playing_sample.half_volume_radius.value, &end_pan.l, &end_pan.r);
    } else {
      // 2D panning
    ...
    // figure out a step to add at each sample so that pan will move smoothly
    // from start to end:
    LR pan = start_pan;
    LR pan_step;
    ...
    if (playing_sample.i >= playing_sample.data.size() ||
        (playing_sample.stopping &&
         playing_sample.volume.value == 0.0f)) { // sample has finished
  ...
  for (uint32_t s = 0; s < MIX_SAMPLES; ++s) {
          max_power = std::max(max_power, (buffer[s].l * buffer[s].l +
  buffer[s].r * buffer[s].r));
  }
  std::cout << "Max Power: " << std::sqrt(max_power) << "; playing samples: " <<
  playing_samples.size() << std::endl; //DEBUG
  */
}
