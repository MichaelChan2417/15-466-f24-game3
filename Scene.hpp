...
/*
 * A scene manages a hierarchical arrangement of transformations (via
 * "Transform").
 *
...
struct Scene {
  struct Transform {
    ...
    // The core function of a transform is to store a transformation in the
    // world:
    glm::vec3 position = glm::vec3(0.0f, 0.0f, 0.0f);
    glm::quat rotation =
        glm::quat(1.0f, 0.0f, 0.0f, 0.0f); // n.b. wxyz init order
    glm::vec3 scale = glm::vec3(1.0f, 1.0f, 1.0f);

    // The transform above may be relative to some parent transform:
    Transform *parent = nullptr;

    // It is often convenient to construct matrices representing this
    // transformation:
    ...
    // since hierarchy is tracked through pointers, copy-constructing a
    // transform  is not advised:
    Transform(Transform const &) = delete;
    // if we delete some constructors, we need to let the compiler know that the
    // default constructor is still okay:
    Transform() = default;
  };
  ...
  struct Drawable {
    ...
    struct Pipeline {
      ...
      GLenum type = GL_TRIANGLES; // what sort of primitive to draw; passed to
                                  // glDrawArrays
      ...
      GLuint OBJECT_TO_CLIP_mat4 =
          -1U; // uniform location for object to clip space matrix
      GLuint OBJECT_TO_LIGHT_mat4x3 = -1U; // uniform location for object to
                                           // light space (== world space) matrix
      GLuint NORMAL_TO_LIGHT_mat3 = -1U; // uniform location for normal to light
                                         // space (== world space) matrix
  ...
  struct Light {
    ...
    Transform *transform;
    // NOTE: directional, spot, and hemisphere lights are directed along their
    // -z axis
  ...
  // the 'on_drawable' callback gives your code a chance to look up mesh data
  // and make Drawables: throws on file format errors
  void load(std::string const &filename,
            std::function<void(Scene &, Transform *, std::string const &)> const
                &on_drawable = nullptr);

  // this function is called to read extra chunks from the scene file after the
  // main chunks are read:
  // this is useful if you, e.g., subclassing scene to represent a game
  // level/area
  virtual void load_extra(std::istream &from, std::vector<char> const &str0,
                          std::vector<Transform *> const &xfh0) {}
